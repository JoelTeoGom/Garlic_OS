...
_ga_printf:
	...
	.global _ga_nice
	@;Par�metros
	@; R0: unsigned int prio
_ga_nice:
	push {r1-r3, lr}
	ldr r1, =_gd_pidz
	ldr r2, [r1]			@; R2 = valor actual de PID + z�calo
	and r2, r2, #0xf 		@; R2 = zocalo actual

	ldr r1, =_gd_pcbs
	mov r3, #28
	mla r1, r3, r2, r1		@; R1 = direcci�n de la PCB del proceso actual
	
	rsb r3, r0, #4			@; R3 = 4 - amabilidad
	str r3, [r1, #24]		@; guardamos la nueva amabilidad en la PCB
	@; cuando se acabe el cuantum total y se efectue un nuevo reset de cuantum, 
	@; se cambiara modificara el quantum total segun la amabilidad del proceso	
	pop {r1-r3, pc}
