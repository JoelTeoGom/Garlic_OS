/*------------------------------------------------------------------------------
	...
	VersiÃƒÂ³n final de GARLIC 2.0
	(multiplexaciÃƒÂ³n, retardar procesos, matar procesos)

------------------------------------------------------------------------------*/
#include <nds.h>
#include <stdlib.h>

#include <garlic_system.h>	// definiciÃƒÂ³n de funciones y variables de sistema
...
/* funciÃƒÂ³n para escribir los porcentajes de uso de la CPU de los procesos de los
		cuatro primeros zÃƒÂ³calos, en el caso que la RSI del TIMER0 haya realizado
		el cÃƒÂ¡lculo */
void porcentajeUso()
{
...
//------------------------------------------------------------------------------
	...
	_gg_iniGrafA();			// inicializar procesadores grÃƒÂ¡ficos
	_gs_iniGrafB();
	...
	_gd_seed = *punixTime;	// inicializar semilla para nÃƒÂºmeros aleatorios con
	_gd_seed <<= 16;		// el valor de tiempo real UNIX, desplazado 16 bits
	
	_gd_pcbs[0].keyName = 0x4C524147;		// "GARL"
	
	if (!_gm_initFS()) {
		_gg_escribir("ERROR: Ã‚Â¡no se puede inicializar el sistema de ficheros!", 0, 0, 0);
		exit(0);
	}
...
//------------------------------------------------------------------------------
	...
	{	
		...
		while (_gd_tickCount < 480)			// esperar 4 segundos mÃƒÂ¡s
		{
			_gp_WaitForVBlank();
			porcentajeUso();
		}
	...
	if (start)
	{
		...
		while (_gd_tickCount < mtics)		// esperar 16 segundos mÃƒÂ¡s
		{
			_gp_WaitForVBlank();
			porcentajeUso();
		}
